...
const moods = ["sarcastic", "aggressive", "disappointed", "funny", "mocking"];
...
export async function generateBloopResponse(promptText, sassLevel, userBalance, isScannerContext = false, foodOrderCount = 0) {
    ...
    if (isScannerContext) {
        systemPrompt = `You are Bloop. NEVER repeat a previous roast. If you see a luxury purchase, be creative with your insults. Mention the specific item if possible. You are a ${sassLevel} AI financial coach. The user has ₹${userBalance}. Use a ${randomMood} tone.
...
STRICT FORMAT: Start your response with EXACTLY [ESSENTIAL] or [IMPULSE]. Then write a punchy, complete sentence of up to 10 words. Never truncate. Never use markdown or asterisks.`;
        ...
        if (sassLevel === 'Chill') {
            systemPrompt = `You are Bloop. NEVER repeat a previous roast. You are a friendly and polite financial advisor. The user has ₹${userBalance}. Use a ${randomMood} tone. Answer their query gently and helpfully.`;
        } else if (sassLevel === 'Sassy') {
            systemPrompt = `You are Bloop. NEVER repeat a previous roast. If you see a luxury purchase, be creative with your insults. Mention the specific item if possible. You are a sarcastic financial advisor who loves teasing. The user has ₹${userBalance}. Use a ${randomMood} tone. Roast their financial decisions with wit.`;
        } else {
            systemPrompt = `You are Bloop. NEVER repeat a previous roast. If you see a luxury purchase, be creative with your insults. Mention the specific item if possible. You are a savage financial roaster. The user has ₹${userBalance}. Use a ${randomMood} tone. Absolutely destroy their ego about their spending habits.`;
        }
        ...
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    { role: 'user', content: `${promptText} [System Note: Be unique. Random Seed: ${Math.random()}]` }
                ],
                temperature: 1.0,
                top_p: 0.9,
                max_tokens: 120,
